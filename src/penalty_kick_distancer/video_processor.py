...
def _run_yolo_inference(model: object, frame):
    """Run inference on CPU while staying compatible with simple test doubles."""

    predict = getattr(model, "predict", None)
    if callable(predict):
        try:
            return predict(frame, verbose=False, device="cpu")
        except TypeError:
            return predict(frame, verbose=False)

    if callable(model):
        try:
            return model(frame, verbose=False, device="cpu")
        except TypeError:
            return model(frame, verbose=False)

    raise VideoProcessingError("Loaded model is not callable and has no predict method.")
...
def _finalize_browser_mp4(raw_path: Path, output_path: Path) -> tuple[Path, list[str]]:
    """Transcode OpenCV output to a browser-friendly MP4 when ffmpeg is available."""

    warnings: list[str] = []
    ffmpeg = shutil.which("ffmpeg")
    if ffmpeg is None:
        warnings.append("FFmpeg was not found; using OpenCV MP4 output without H.264 transcode.")
    ...
    try:
        ...
        warnings.append(f"FFmpeg transcode could not start: {exc}. Using OpenCV MP4 output.")
        return raw_path, warnings

    if completed.returncode != 0 or not output_path.exists() or output_path.stat().st_size == 0:
        detail = completed.stderr.strip().splitlines()[-1] if completed.stderr.strip() else "unknown error"
        warnings.append(f"FFmpeg H.264 transcode failed: {detail}. Using OpenCV MP4 output.")
        return raw_path, warnings
...
def process_video(
    ...
    raw_output_video_path = output_dir / "annotated_cv2.mp4"
    ...
    try:
        while True:
            ...
            if should_infer:
                result = _run_yolo_inference(model, frame)
            ...
            frame_index += 1
            if processed_count >= config.max_frames:
                break
    finally:
        cap.release()
        writer.release()

    final_output_video_path, encoding_warnings = _finalize_browser_mp4(raw_output_video_path, output_video_path)
    ...
    metadata = {
        ...
        "output_video_path": str(final_output_video_path),
    ...
    return str(final_output_video_path), best_frame_path, metrics, per_frame, metadata, plot_path
