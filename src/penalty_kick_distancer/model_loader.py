...
def load_yolo_model(model_path: str | None = None) -> LoadedModel:
    ...
    for candidate in _candidate_local_paths(model_path):
        if candidate.exists():
            ...
            if hasattr(model, "to"):
                model.to("cpu")
            return LoadedModel(model=model, model_path=str(candidate), source="local")

    hub_path = _download_from_hub()
    if hub_path is not None:
        model = YOLO(str(hub_path))
        if hasattr(model, "to"):
            model.to("cpu")
        return LoadedModel(model=model, model_path=str(hub_path), source="huggingface_hub")

    try:
        print(
            "WARNING: No fine-tuned Penalty-Kick Distancer model found. "
            "Loading yolov8n.pt fallback demo mode."
        )
